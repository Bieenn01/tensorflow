...
absl::Status LocalExecutable::ValidateExecutionOptions(
  ...
  if (run_options.stream() != nullptr) {
    ...
    // If the physical device ordinal is specified, it should match the ordinal
    // of the device the stream belongs to.
    ...
    if (run_options.physical_device_ordinal() != -1) {
      ...
      if (run_options.device_ordinal() == -1) {
        return InvalidArgument(
            "The logical device ordinal is required if the physical "
            "device ordinal is specified.");
      }
